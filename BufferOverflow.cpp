...
// Try "1234567812345678Devil I am. Ha Ha" as a password for MSVC debug x86
// build. If doesn't work, investigate why :)
...
#define USER_INPUT_MAX_LENGTH 8
  ...
  printf("login as: ");
  fflush(stdout);
  // gets(userName); // use scanf("%s", userName); if gets fails with identifier
  // not found
  scanf("%s", userName);
  ...
  fflush(stdout);
  // gets(passwd);
  scanf("%s", passwd); // use scanf("%s", passwd); if gets fails with identifier
                       // not found
...
void demoAdjacentMemoryOverflow(char *userName, char *password) {
  ...
  strncpy(buf, userName,
          sizeof(buf)); // We will copy only characters which fits into buf
...
int main() {
  ...
  // Eclipse, Debug - work
  // VS, Release - stack protection nonce example - does not work due to canary
  // word
  ...
  demoDataTypeOverflow(1073742, &data, 5);	// 1073742 * 4000 => only 704 B
  allocated
  /**/
  return 0;
}
